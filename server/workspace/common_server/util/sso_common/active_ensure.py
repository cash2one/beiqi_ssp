...
def validate(api_key, auth_cipher, sign, ver=1):
    ...
    if not api_ob:
        #api_key未找到
        gen_log.warn('no api_key {0}'.format(api_key))
        return 4
    ...
    if not (acc and code and pwd):
        gen_log.warn('params lost in {0}'.format(plain))
        return 5

    acc = acc.split('@')[0]
    if not is_mobile(acc):
        gen_log.warn('acc not mobile: {0}'.format(acc))
        return 3
    ...
    if not (lt_acc and has_pwd):
        gen_log.warn('not lt_acc, active refuse: {0}'.format(api_key))
        return 1
