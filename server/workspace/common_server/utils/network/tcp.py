...
class LevelDBRpcHandler(RPCServer):
    """
    LevelDBRpc回调处理类
    """
    @gevent_adaptor()
    ...
    def forward(self, params):
        ...
        :return:
        """
        logger.info("LevelDBRpcHandler::forward, params:{0}".format(params))


class LevelDBRpcServer(RpcServer):
    """
    LevelDBRpc服务
    """
    def __init__(self, port, tcp_handler=LevelDBRpcHandler):
        super(LevelDBRpcServer, self).__init__(PT_TCP, port, StreamServer(('', port), tcp_handler))


class LevelDBRpcClient(TcpRpcClient):
    """
    LevelDBRpc客户端
    """
    def __init__(self, host, port):
        super(LevelDBRpcClient, self).__init__(host, port)
