...
class BeiqiSSOHandler(HttpRpcHandler):
    ...
    生成SSO认证token
        ...
        if len(username) == 66:
            ...
            saved_ts = GDevRdsInts.execute(get_tk_time(sn))
            if saved_ts == ts:
                logger.debug('ts={0} the same with saved_ts'.format(ts))
                self.send_error(400)
                return

            GDevRdsInts.execute(set_tk_time(sn, ts))
            ...
            return gen_token(api_ob.get('s'), sn, 1)
        ...
        if len(gid) == 6:
            ...
            if sn:
                ...
                return gen_token(api_ob.get('s'), username, 1)
            else:
        ...
        expect_pwd = GAccRdsInts.execute(get_pwd(username))
        ...
        if expect_pwd is not None:
            ...
            expect_pwd = DBBeiqiSspInst.query(sql, username)
            if len(expect_pwd) == 0:
